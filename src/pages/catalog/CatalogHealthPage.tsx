...
export default function CatalogHealthPage() {
  ...
  return (
    <ChannablePageWrapper
      ...
      description={`${metrics?.total || 0} produits — Score global : ${metrics?.globalScore || 0}%`}
      heroImage="products"
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {detailedKPIs.map((kpi) => (
          <StatCard key={kpi.label} label={kpi.label} value={`${kpi.value}%`} sub={`${kpi.count} produits`} icon={kpi.icon} color={kpi.color} />
        ))}
      </div>

      <Tabs defaultValue="ai" className="space-y-6">
        <TabsList className="grid w-full max-w-md grid-cols-2">
          <TabsTrigger value="ai" className="flex items-center gap-2"><Brain className="h-4 w-4" />Intelligence IA<Sparkles className="h-3 w-3 text-violet-500" /></TabsTrigger>
          <TabsTrigger value="details" className="flex items-center gap-2"><HeartPulse className="h-4 w-4" />Détails</TabsTrigger>
        </TabsList>
        <TabsContent value="ai"><CatalogHealthAIPanel /></TabsContent>
        <TabsContent value="details">
          <div className="space-y-6">
            <Card>
              <CardHeader><CardTitle className="text-lg">Détails de complétude</CardTitle></CardHeader>
              <CardContent>
                  ...
                  {completenessDetails.map((detail) => {
                    ...
                    return (
                      <div key={detail.label} className="space-y-2">
                        <div className="flex items-center gap-2"><detail.icon className="h-4 w-4 text-muted-foreground" /><span className="text-sm font-medium">{detail.label}</span></div>
                        <div className="flex items-baseline gap-2"><span className="text-2xl font-bold">{detail.value}</span><span className="text-sm text-muted-foreground">/ {detail.total}</span></div>
