...
const supportedPlatforms = [
  'aliexpress', 'amazon', 'ebay', 'temu', 'wish', 'cjdropshipping', 'bigbuy', 'banggood', 'shein', 'etsy'
]
...
export default function AutoDSImportPage() {
  ...
  const processUrlQueue = async () => {
    ...
    for (const item of queuedUrls) {
      ...
      setQueuedUrls(prev => prev.map(q => 
        q.id === item.id ? { ...q, status: 'loading' as const } : q
      ))
      ...
      try {
        ...
        setQueuedUrls(prev => prev.map(q => 
          q.id === item.id ? { ...q, status: 'success' as const, preview: data.data } : q
        ))
      } catch (err) {
        setQueuedUrls(prev => prev.map(q => 
          q.id === item.id ? { 
    ...
    <>
      ...
      <div className="container max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold">Import Rapide AutoDS-Style</h1>
                      ...
                      {queuedUrls.map((item) => (
                        <div 
                          ...
                          className={cn(
                            ...
                            item.status === 'success' && "bg-green-50 dark:bg-green-950/20 border-green-200",
                            item.status === 'error' && "bg-red-50 dark:bg-red-950/20 border-red-200",
                            item.status === 'loading' && "bg-blue-50 dark:bg-blue-950/20 border-blue-200"
                          )}
                        >
                          {/* Preview Image */}
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          value={imageProductInfo.category}
                          onChange={(e) => setImageProductInfo(prev => ({ ...prev, category: e.target.value }))}
                        />
                      </div>
                      <div className="space-y-2">
