...
export default function ConnectorsPage() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (success) {
        toast({ title: 'Connexion réussie', description: `Connecté à ${allConnectors.find(c => c.id === selectedConnector)?.name}` })
  ...
  const fieldLabels: Record<string, string> = {
    ...
    shop_domain: 'Shop Domain (ex: myshop.myshopify.com)', site_url: 'Site URL',
    marketplace_id: 'Marketplace ID', seller_id: 'Seller ID', shop_id: 'Shop ID',
    user_id: 'User ID', api_url: 'API URL', base_url: 'Base URL',
    store_hash: 'Store Hash', shop_url: 'Shop URL',
  }

  const renderCredentialFields = (fields: string[]) => fields.map((field) => (
    <div key={field} className="space-y-2">
      <Label htmlFor={field}>{fieldLabels[field] || field}</Label>
      <Input id={field} placeholder={`Entrez votre ${fieldLabels[field] || field}`} value={credentials[field] || ''} onChange={(e) => setCredentials({ ...credentials, [field]: e.target.value })} />
    </div>
  ))
        ...
        <CardContent className="pt-6">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <Input placeholder="Rechercher un connecteur..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9" />
            </div>
            <div className="flex gap-2">
              <Button variant={categoryFilter === 'all' ? 'default' : 'outline'} onClick={() => setCategoryFilter('all')}>Tous</Button>
              <Button variant={categoryFilter === 'marketplace' ? 'default' : 'outline'} onClick={() => setCategoryFilter('marketplace')}>Marketplaces</Button>
              <Button variant={categoryFilter === 'ecommerce' ? 'default' : 'outline'} onClick={() => setCategoryFilter('ecommerce')}>E-commerce</Button>
              <Button variant={categoryFilter === 'supplier' ? 'default' : 'outline'} onClick={() => setCategoryFilter('supplier')}>Fournisseurs</Button>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              <CardContent>
                {isConnected ? (
                  <div className="space-y-2">
                    <Button className="w-full" variant="outline" size="sm"><Settings className="h-4 w-4 mr-2" />Configurer</Button>
                    <Button className="w-full" variant="outline" size="sm"><RefreshCw className="h-4 w-4 mr-2" />Synchroniser</Button>
                    <Button className="w-full" variant="destructive" size="sm"><Trash2 className="h-4 w-4 mr-2" />Déconnecter</Button>
                  </div>
                ) : (
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button className="w-full" onClick={() => setSelectedConnector(connector.id)}><Plug className="h-4 w-4 mr-2" />Connecter</Button>
                    </DialogTrigger>
                    <DialogContent className="max-w-md">
                      <DialogHeader>
                        <DialogTitle>Connecter {connector.name}</DialogTitle>
                        <DialogDescription>Entrez vos identifiants pour connecter {connector.name}</DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4 py-4">{renderCredentialFields(connector.fields)}</div>
                      <div className="flex justify-end gap-2">
                        <Button variant="outline" onClick={() => { setSelectedConnector(null); setCredentials({}) }}>Annuler</Button>
                        <Button onClick={handleConnect} disabled={connecting}>
                          {connecting ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Connexion...</>) : 'Connecter'}
                        </Button>
                      </div>
                    </DialogContent>
                  </Dialog>
      ...
      {filteredConnectors.length === 0 && (
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-muted-foreground">Aucun connecteur trouvé pour "{searchTerm}"</p>
          </CardContent>
        </Card>
      )}
    </ChannablePageWrapper>
