...
export class Connector1688 extends BaseConnector {
  constructor(credentials: any) {
    super(credentials, 'https://open.1688.com');
  }
  ...
  async validateCredentials(): Promise<boolean> {
    return !!this.credentials.access_token && !!this.credentials.app_key;
  }
  ...
  private mapProduct(p: any): SupplierProduct {
    ...
    return {
      ...
      currency: p.currency || 'CNY', stock: p.stock_quantity || 0, images,
      category: attrs.category || '', brand: attrs.brand || '',
      supplier: { id: '1688', name: '1688.com', sku: p.external_product_id || p.id },
      attributes: attrs
    };
  }
  ...
  async updateInventory(products: SupplierProduct[]): Promise<SyncResult> {
    ...
    for (const product of products) {
      const { error } = await client.from('supplier_products').upsert({
        user_id: user.id, source: '1688', external_product_id: product.sku,
