...
export class TemuConnector extends BaseConnector {
  constructor(credentials: any) {
    super(credentials, 'https://api.temu.com');
  }
  ...
  private mapProduct(p: any): SupplierProduct {
    ...
    return {
      ...
      supplier: { id: 'temu', name: 'Temu', sku: p.external_product_id || p.id },
      attributes: attrs
    };
  }
  ...
  async updateInventory(products: SupplierProduct[]): Promise<SyncResult> {
    ...
    for (const product of products) {
      const { error } = await client.from('supplier_products').upsert({
        user_id: user.id, source: 'temu', external_product_id: product.sku,
