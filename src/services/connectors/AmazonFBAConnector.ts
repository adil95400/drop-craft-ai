...
export class AmazonFBAConnector extends BaseConnector {
  constructor(credentials: any) {
    super(credentials, 'https://sellingpartnerapi-eu.amazon.com');
  }
  ...
  async validateCredentials(): Promise<boolean> {
    return !!this.credentials.access_token && !!this.credentials.seller_id;
  }
  ...
  async fetchProduct(sku: string): Promise<SupplierProduct | null> {
    try {
      ...
      const { data, error } = await client
        ...
        .single();
      return error || !data ? null : this.mapProduct(data);
    } catch { return null; }
  }

  private mapProduct(p: any): SupplierProduct {
    ...
    return {
      id: p.id, sku: p.external_product_id || p.id, title: p.title || '',
      description: p.description || '', price: p.price || 0, costPrice: p.cost_price || 0,
      currency: p.currency || 'EUR', stock: p.stock_quantity || 0, images,
      category: attrs.category || '', brand: attrs.brand || '',
      supplier: { id: 'amazon_fba', name: 'Amazon FBA', sku: p.external_product_id || p.id },
      attributes: { ...attrs, fulfillment: 'FBA' }
    };
  }
  ...
  async updateInventory(products: SupplierProduct[]): Promise<SyncResult> {
    ...
    for (const product of products) {
      const { error } = await client.from('supplier_products').upsert({
        user_id: user.id, source: 'amazon_fba', external_product_id: product.sku,
        ...
        attributes: { category: product.category, brand: product.brand, fulfillment: 'FBA', ...product.attributes }
      });
