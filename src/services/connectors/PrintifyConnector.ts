...
export class PrintifyConnector extends BaseConnector {
  constructor(credentials: any) {
    super(credentials, 'https://api.printify.com/v1');
  }
  ...
  private mapProduct(p: any): SupplierProduct {
    ...
    return {
      ...
      currency: p.currency || 'USD', stock: p.stock_quantity || 0, images,
      category: attrs.category || 'Print on Demand', brand: attrs.brand || '',
      supplier: { id: 'printify', name: 'Printify', sku: p.external_product_id || p.id },
      attributes: attrs
    };
  }
  ...
  async updateInventory(products: SupplierProduct[]): Promise<SyncResult> {
    ...
    for (const product of products) {
      const { error } = await client.from('supplier_products').upsert({
        user_id: user.id, source: 'printify', external_product_id: product.sku,
