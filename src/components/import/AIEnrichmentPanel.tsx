...
export default function AIEnrichmentPanel({ selectedProductIds, onComplete }: AIEnrichmentPanelProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'running': case 'processing': return <Badge className="bg-info/20 text-blue-400 border-info/30"><Loader2 className="w-3 h-3 mr-1 animate-spin" />En cours</Badge>
      case 'completed': return <Badge className="bg-success/20 text-green-400 border-success/30"><CheckCircle2 className="w-3 h-3 mr-1" />Terminé</Badge>
