...
const StatusBadge = ({ status, reviewStatus }: { status: string; reviewStatus?: string }) => {
  if (reviewStatus === 'approved') {
    return <Badge className="bg-success/10 text-success dark:bg-green-900 dark:text-green-200">Approuvé</Badge>;
  }
  if (reviewStatus === 'rejected') {
    return <Badge variant="destructive">Rejeté</Badge>;
  }
  if (status === 'published') {
    return <Badge className="bg-info/10 text-blue-800 dark:bg-blue-900 dark:text-blue-200">Publié</Badge>;
  }
...
const PlatformBadge = ({ platform }: { platform?: string }) => {
  ...
  const platformColors: Record<string, string> = {
    ...
    amazon: 'bg-warning/10 text-yellow-800',
    ebay: 'bg-info/10 text-blue-800',
