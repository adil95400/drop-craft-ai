      ...
      {extensions.map((extension) => (
        <Card key={extension.id}>
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-4 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold truncate">{extension.display_name}</h3>
                    <Badge 
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <div className="flex items-center gap-1">
                      {extension.last_sync_at ? (
                        <>
                          <CheckCircle2 className="w-4 h-4 text-green-500" />
                          <span>Dernière sync: {new Date(extension.last_sync_at).toLocaleDateString()}</span>
                        </>
                      ) : (
                        <>
              ...
              <div className="flex items-center gap-3 ml-4">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleUninstall(extension.id, extension.display_name)}
                >
            ...
            {extension.status === 'active' && extension.configuration && (
              <div className="mt-4 pt-4 border-t">
                <div className="text-xs text-muted-foreground mb-2">Configuration actuelle:</div>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(extension.configuration).slice(0, 3).map(([key, value]) => (
                    <Badge key={key} variant="outline" className="text-xs">
                      {key}: {String(value)}
                    </Badge>
                  ))}
                  {Object.keys(extension.configuration).length > 3 && (
                    <Badge variant="outline" className="text-xs">
                      +{Object.keys(extension.configuration).length - 3} autres
                    </Badge>
                  )}
                </div>
              </div>
