...
export const SyncLogsTable = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'success':
        return <Badge className="bg-success/10 text-success border-success/20"><CheckCircle className="w-3 h-3 mr-1" />Réussi</Badge>
      case 'error':
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'products': return 'bg-info/10 text-blue-800 border-info/20'
      case 'orders': return 'bg-success/10 text-success border-success/20'
