...
import { Search, Bot, ShoppingCart, BarChart3, Truck, Upload, Trophy, TrendingUp, Zap, Users, Brain, Shield, Plug, Settings, ChevronDown, Package, Sparkles, Crown, Calculator, Megaphone, FileText, Globe, Store, Puzzle, GitCompare, Database, ShoppingBag, GraduationCap, HelpCircle, Activity, Building2, Building, Star, LayoutDashboard, Link, Layers, LineChart, FileSpreadsheet, Code, Clock, History, Download, Key, UserPlus, Mail, Phone, Calendar, MessageSquare, Type, DollarSign, Heart, BookOpen } from "lucide-react";
...
const ShopoptiLogo = memo(() => <div className="flex items-center gap-3 px-2 group cursor-pointer">
...
export function AppSidebar() {
  ...
  const [openGroups, setOpenGroups] = useState<string[]>(moduleGroups.map(g => g.title) // Tous les groupes ouverts par défaut
  );
  ...
  const activeModule = useMemo(() => {
    ...
    return availableModules.find(module => currentPath === module.route || currentPath.startsWith(module.route + '/'));
  ...
  const filteredModuleGroups = useMemo(() => {
    ...
    return moduleGroups.map(group => ({
      ...
      modules: group.modules.filter(moduleId => {
        ...
        return module && (module.name.toLowerCase().includes(query) || module.description.toLowerCase().includes(query));
      })
  ...
  return <Sidebar collapsible="icon" className="border-r border-border/50 bg-gradient-to-b from-card/80 to-background/60 backdrop-blur-xl backdrop-saturate-150 shadow-xl transition-all duration-300">
      <SidebarHeader className="border-b border-border/30 bg-gradient-to-br from-background/95 via-muted/20 to-background/90 backdrop-blur-2xl shadow-sm">
        <div className="px-3 py-4 animate-fade-in">
          {state !== "collapsed" ? <div className="space-y-3">
              <div className="relative">
              ...
              {isAdminBypass && <div className="relative overflow-hidden rounded-lg">
        ...
        {state !== "collapsed" && <div className="px-3 pb-4 animate-fade-in">
            <div className="relative group">
              ...
              <Input placeholder="Rechercher un module..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="relative pl-9 pr-4 bg-background/60 border-border/40 backdrop-blur-sm rounded-lg transition-all duration-200 focus:bg-background focus:border-primary/50 focus:shadow-md focus:shadow-primary/10 hover:border-border/60" />
            </div>
          </div>}
      </SidebarHeader>
      ...
      <SidebarContent className="px-3 py-3 custom-scrollbar">
        {/* Section Favoris */}
        {favorites.length > 0 && <SidebarGroup className="mb-3">
            ...
            <SidebarGroupContent>
              ...
              return <SidebarMenuItem key={module.id}>
                      <SidebarMenuButton onClick={() => handleNavigate(module.route, module.id)} tooltip={state === "collapsed" ? module.name : undefined} className={cn("w-full justify-start transition-all duration-300 group relative overflow-hidden rounded-lg", active ? "bg-gradient-to-r from-primary to-primary/90 text-primary-foreground shadow-lg shadow-primary/30 scale-[1.02]" : "hover:bg-accent/80 hover:text-accent-foreground hover:shadow-md hover:scale-[1.01] backdrop-blur-sm", "before:absolute before:inset-0 before:bg-gradient-to-r before:from-transparent before:via-white/10 before:to-transparent before:translate-x-[-100%] hover:before:translate-x-[100%] before:transition-transform before:duration-700")} isActive={active}>
                        <Icon className={cn("h-4 w-4 transition-transform duration-300", active && "scale-110 drop-shadow-lg")} />
                        {state !== "collapsed" && <div className="flex items-center justify-between w-full gap-2">
        ...
        const groupModules = group.modules.map(id => MODULE_REGISTRY[id]).filter(m => m && isModuleEnabled(m.id));
        if (groupModules.length === 0) return null;
        return <Collapsible key={group.title} open={openGroups.includes(group.title)} onOpenChange={() => toggleGroup(group.title)}>
              <SidebarGroup>
                  ...
                  <SidebarGroupLabel className="group/label w-full hover:bg-gradient-to-r hover:from-accent/80 hover:to-accent/40 hover:text-accent-foreground rounded-lg transition-all duration-300 cursor-pointer hover:shadow-md hover:shadow-primary/5 backdrop-blur-sm border border-transparent hover:border-border/30">
                    <div className="flex items-center justify-between w-full px-1">
                      {state !== "collapsed" && <>
                          ...
                          <div className="relative">
                            <ChevronDown className={cn("h-4 w-4 transition-all duration-300 group-hover/label:text-primary group-hover/label:scale-110", openGroups.includes(group.title) && "rotate-180")} />
                ...
                <CollapsibleContent className="animate-accordion-down">
                  <SidebarGroupContent>
                    ...
                    return <SidebarMenuItem key={module.id} className="animate-fade-in">
                              {!hasSubItems ? <SidebarMenuButton onClick={() => handleNavigate(module.route, module.id)} tooltip={state === "collapsed" ? module.name : undefined} className={cn("w-full justify-start transition-all duration-300 group relative overflow-hidden rounded-lg", active ? "bg-gradient-to-r from-primary to-primary/90 text-primary-foreground shadow-lg shadow-primary/30 scale-[1.02] border border-primary/20" : "hover:bg-gradient-to-r hover:from-accent/80 hover:to-accent/40 hover:text-accent-foreground hover:shadow-md hover:scale-[1.01] backdrop-blur-sm border border-transparent hover:border-border/30", !accessible && "opacity-50 cursor-not-allowed hover:scale-100", "before:absolute before:inset-0 before:bg-gradient-to-r before:from-transparent before:via-white/20 before:to-transparent before:translate-x-[-100%] hover:before:translate-x-[100%] before:transition-transform before:duration-700")} disabled={!accessible} isActive={active}>
                                              <Icon className={cn("h-4 w-4 transition-all duration-300", active && "scale-110 drop-shadow-lg")} />
                                               {state !== "collapsed" && <div className="flex items-center justify-between w-full gap-2">
                                                  ...
                                                  <div className="flex items-center gap-1.5">
                                                    ...
                                                    {module.minPlan !== 'standard' && <Badge variant="secondary" className={cn("text-xs h-5 px-2 font-bold transition-all duration-200 group-hover:scale-105 shadow-sm", getBadgeVariant(module.minPlan))}>
                                                        {module.minPlan === 'ultra_pro' ? 'ULTRA' : 'PRO'}
                                                      </Badge>}
                                                  </div>
                                                </div>}
                              </SidebarMenuButton> : <Collapsible open={subMenuOpen} onOpenChange={() => toggleSubMenu(module.id)}>
                                  <CollapsibleTrigger asChild>
                                    <SidebarMenuButton tooltip={state === "collapsed" ? module.name : undefined} className={cn("w-full justify-start transition-all duration-300 group relative overflow-hidden rounded-lg", active ? "bg-gradient-to-r from-primary to-primary/90 text-primary-foreground shadow-lg shadow-primary/30 scale-[1.02] border border-primary/20" : "hover:bg-gradient-to-r hover:from-accent/80 hover:to-accent/40 hover:text-accent-foreground hover:shadow-md hover:scale-[1.01] backdrop-blur-sm border border-transparent hover:border-border/30", !accessible && "opacity-50 cursor-not-allowed hover:scale-100", "before:absolute before:inset-0 before:bg-gradient-to-r before:from-transparent before:via-white/20 before:to-transparent before:translate-x-[-100%] hover:before:translate-x-[100%] before:transition-transform before:duration-700")} disabled={!accessible} isActive={active}>
                                      <Icon className={cn("h-4 w-4 transition-all duration-300", active && "scale-110 drop-shadow-lg")} />
                                      {state !== "collapsed" && <div className="flex items-center justify-between w-full gap-2">
                                          <span className="truncate transition-transform duration-200 group-hover:translate-x-0.5 font-medium">
                                            {module.name}
                                          ...
                                          <div className="flex items-center gap-1.5">
                                            <FavoriteButton moduleId={module.id} size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity" />
                                            {module.minPlan !== 'standard' && <Badge variant="secondary" className={cn("text-xs h-5 px-2 font-bold transition-all duration-200 group-hover:scale-105", getBadgeVariant(module.minPlan))}>
                                                {module.minPlan === 'ultra_pro' ? 'ULTRA' : 'PRO'}
                                              </Badge>}
                                            <ChevronDown className={cn("h-4 w-4 transition-all duration-300 text-muted-foreground", subMenuOpen && "rotate-180 text-primary scale-110")} />
                                          </div>
                                        </div>}
                                    </SidebarMenuButton>
                                  </CollapsibleTrigger>
                                  
                                  {state !== "collapsed" && <CollapsibleContent className="overflow-hidden transition-all duration-300 data-[state=open]:animate-accordion-down data-[state=closed]:animate-accordion-up">
                              ...
                              return <div key={subModule.id} className="animate-fade-in" style={{
                                animationDelay: `${idx * 30}ms`
                              }}>
                                              <SidebarMenuButton onClick={() => handleNavigate(subModule.route, module.id)} className={cn("w-full justify-start text-sm transition-all duration-300 group/sub relative rounded-lg overflow-hidden", subActive ? "bg-gradient-to-r from-primary/20 to-primary/10 text-primary font-semibold border border-primary/30 shadow-sm shadow-primary/20" : "hover:bg-gradient-to-r hover:from-accent/60 hover:to-accent/30 hover:border hover:border-border/30 hover:translate-x-1 hover:shadow-sm", "pl-2.5")} isActive={subActive}>
                                                <SubIcon className={cn("h-3.5 w-3.5 transition-all duration-300", subActive && "text-primary scale-110 drop-shadow-sm")} />
                                                <span className="truncate">{subModule.name}</span>
                                                {subActive && <div className="ml-auto flex items-center gap-1">
