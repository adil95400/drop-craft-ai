...
export interface CallRecord {
  id: string; user_id: string; customer_name: string; customer_phone: string;
  customer_email?: string; type: 'incoming' | 'outgoing' | 'missed';
  status: 'completed' | 'missed' | 'busy' | 'scheduled' | 'in_progress';
  duration: number; date: string; notes?: string; follow_up?: string; outcome?: string; created_at: string;
}

interface CallStats {
  totalCalls: number; completedToday: number; missedCalls: number;
  scheduledCalls: number; avgDuration: number; totalDurationToday: number;
}
...
export function useCRMCalls() {
  ...
  const [stats, setStats] = useState<CallStats>({ totalCalls: 0, completedToday: 0, missedCalls: 0, scheduledCalls: 0, avgDuration: 0, totalDurationToday: 0 });
  ...
  const calculateStats = (calls: CallRecord[]) => {
    ...
    setStats({
      ...
      avgDuration: completedCalls.length > 0 ? completedCalls.reduce((acc, c) => acc + c.duration, 0) / completedCalls.length : 0,
  ...
  const fetchCalls = useCallback(async () => {
    ...
    try {
      const { data, error } = await (supabase.from('crm_calls') as any)
        .select('*').eq('user_id', user.id).order('date', { ascending: false });
      if (error) throw error;
      const callData = (data || []) as CallRecord[];
      setCalls(callData);
  ...
  const logCall = useCallback(async (callData: Partial<CallRecord>) => {
    ...
    try {
      const { data, error } = await (supabase.from('crm_calls') as any)
        .insert({ ...callData, user_id: user.id }).select().single();
      if (error) throw error;
  ...
  const scheduleCall = useCallback(async (callData: { customer_name: string; customer_phone: string; scheduled_date: string; notes?: string }) => {
  ...
  const updateCall = useCallback(async (id: string, updates: Partial<CallRecord>) => {
    try {
      const { error } = await (supabase.from('crm_calls') as any).update(updates).eq('id', id);
      if (error) throw error;
  ...
  const deleteCall = useCallback(async (id: string) => {
    try {
      const { error } = await (supabase.from('crm_calls') as any).delete().eq('id', id);
      if (error) throw error;
