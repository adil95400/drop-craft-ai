...
export interface SiteHealthData {
  overall: number; seo: number; images: number; content: number; translations: number;
  lastOptimization: string; optimizationCount: number; improvement: number; nextOptimization: string;
}
...
export function useSiteHealth() {
  const { data: siteHealth, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<SiteHealthData> => DEFAULT_HEALTH,
