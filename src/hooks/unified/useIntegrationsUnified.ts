/**
 * useIntegrationsUnified - Hook unifié pour la gestion des intégrations
 * Consolide: useIntegrations, useIntegrationsData, useRealIntegrations
 * 
 * Fonctionnalités:
 * - Realtime subscriptions
 * - Connexions spécifiques (Shopify, AliExpress, BigBuy)
...
export function useIntegrationsUnified(options: UseIntegrationsUnifiedOptions = {}) {
  ...
  const { data: integrations = [], isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((item: any): UnifiedIntegration => ({
        id: item.id,
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Succès", description: "Intégration ajoutée avec succès" })
    },
    onError: (error: Error) => {
      toast({ title: "Erreur", description: error.message || "Impossible d'ajouter l'intégration", variant: "destructive" })
    }
  })
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      invalidateIntegrationQueries()
      toast({ title: "Succès", description: "Intégration mise à jour" })
    },
    onError: () => {
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => {
      toast({ title: "Erreur", description: "Impossible de supprimer l'intégration", variant: "destructive" })
    }
  })
  ...
  const connectMutation = useMutation({
    mutationFn: async ({ template, credentials }: { template: IntegrationTemplate | any; credentials?: Record<string, string> }) => {
      return addMutation.mutateAsync({ 
        template, 
        config: { connection_status: 'connected', is_active: true },
        credentials 
      })
    }
  })
  ...
  const disconnectMutation = useMutation({
    mutationFn: async (id: string) => {
      return updateMutation.mutateAsync({ 
        id, 
        updates: { connection_status: 'disconnected', is_active: false } 
      })
    }
  })
  ...
  const syncMutation = useMutation({
    ...
    onError: () => {
      toast({ title: "Erreur", description: "Impossible de lancer la synchronisation", variant: "destructive" })
    }
  })
  ...
  const syncProductsMutation = useMutation({
    mutationFn: async (integrationId: string) => {
      const { data, error } = await supabase.functions.invoke('shopify-sync', {
        body: { integrationId, type: 'products' }
      })
    ...
    onError: () => {
      toast({ title: "Erreur", description: "Impossible de synchroniser les produits", variant: "destructive" })
    }
  })
  ...
  const syncOrdersMutation = useMutation({
    mutationFn: async (integrationId: string) => {
      const { data, error } = await supabase.functions.invoke('shopify-sync', {
        body: { integrationId, type: 'orders' }
      })
    ...
    onError: () => {
      toast({ title: "Erreur", description: "Impossible de synchroniser les commandes", variant: "destructive" })
    }
  })
  ...
  const connectShopifyMutation = useMutation({
    mutationFn: async (credentials: { shop_domain?: string; access_token?: string }) => {
      return addMutation.mutateAsync({
        ...
        config: { 
          store_url: credentials.shop_domain,
  ...
  const connectAliExpressMutation = useMutation({
    mutationFn: async (credentials: any) => {
      return addMutation.mutateAsync({
        template: { id: 'aliexpress', name: 'AliExpress' },
        config: { connection_status: 'connected', is_active: true, ...credentials }
      })
    }
  })
  ...
  const connectBigBuyMutation = useMutation({
    mutationFn: async (credentials: any) => {
      return addMutation.mutateAsync({
        template: { id: 'bigbuy', name: 'BigBuy' },
        config: { connection_status: 'connected', is_active: true, ...credentials }
      })
    }
  })
  ...
  return {
    ...
    stats,
    syncLogs: [] as SyncLog[], // Placeholder - sync_logs table not in schema
    ...
    invalidate: invalidateIntegrationQueries
  }
}
