...
-- Créer le type de plan d'abord
CREATE TYPE public.plan_type AS ENUM ('standard', 'pro', 'ultra_pro');

-- Sauvegarder les données existantes, supprimer la colonne et la recréer
ALTER TABLE public.profiles ADD COLUMN new_plan plan_type DEFAULT 'standard';

-- Migrer les données selon les correspondances
UPDATE public.profiles SET new_plan = 
  CASE 
    WHEN plan = 'ultra_pro' THEN 'ultra_pro'::plan_type
    WHEN plan = 'pro' THEN 'pro'::plan_type
