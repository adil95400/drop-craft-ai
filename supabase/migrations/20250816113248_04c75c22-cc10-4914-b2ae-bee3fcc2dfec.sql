...
-- 3. Create edge function helper for secure credential verification
...
-- Create new secure policies that restrict access to sensitive fields
...
-- Allow users to update non-sensitive fields only (using a trigger-based approach)
...
-- 5. Create a trigger to prevent updates to sensitive fields by regular users
CREATE OR REPLACE FUNCTION public.prevent_sensitive_supplier_updates()
RETURNS TRIGGER AS $$
BEGIN
  -- Only allow service role to update sensitive fields
  IF current_setting('role') != 'service_role' THEN
    -- Prevent updates to sensitive fields
    IF OLD.api_key IS DISTINCT FROM NEW.api_key OR
       OLD.encrypted_credentials IS DISTINCT FROM NEW.encrypted_credentials OR
       OLD.contact_email IS DISTINCT FROM NEW.contact_email OR
       OLD.contact_phone IS DISTINCT FROM NEW.contact_phone THEN
      RAISE EXCEPTION 'Access denied: Cannot update sensitive supplier fields';
    END IF;
  END IF;
...
-- Apply the trigger
CREATE TRIGGER prevent_sensitive_supplier_updates_trigger
  BEFORE UPDATE ON public.suppliers
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_sensitive_supplier_updates();
