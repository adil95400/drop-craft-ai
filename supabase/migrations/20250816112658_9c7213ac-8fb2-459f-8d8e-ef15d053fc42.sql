...
-- Prevent any DELETE operations on financial data for audit compliance
-- No DELETE policy = no deletes allowed

-- 2. Enhance subscriptions table security
-- Current policies are better but need tightening

-- Ensure subscriptions table has proper constraints
-- Add constraint to ensure user_id is never null for data integrity
ALTER TABLE public.subscriptions 
ALTER COLUMN user_id SET NOT NULL;

-- Add constraint to ensure subscribers.user_id is not null when provided
-- This prevents orphaned subscription records
UPDATE public.subscribers SET user_id = NULL WHERE user_id = '';
ALTER TABLE public.subscribers 
ADD CONSTRAINT subscribers_user_id_check 
CHECK (user_id IS NOT NULL OR user_id IS NULL);

-- 3. Add audit logging for sensitive subscription data access
...
-- Add audit triggers to both tables
