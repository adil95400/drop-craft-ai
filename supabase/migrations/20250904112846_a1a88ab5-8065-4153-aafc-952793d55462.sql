-- Phase 1: Amélioration des données réelles
-- Ajouter plus de données de test réalistes

-- 1. Ajouter des commandes réalistes avec différents statuts
INSERT INTO public.orders (shopify_order_id, customer_id, total_amount, currency, status, created_at)
SELECT 
...
FROM customers c, generate_series(1, 50) -- 50 nouvelles commandes
WHERE c.id IN (SELECT id FROM customers LIMIT 10);
...
-- 3. Créer des données d'avis clients réalistes
INSERT INTO public.reviews (product_id, customer_id, rating, comment, created_at)
SELECT 
  p.id,
  c.id,
...
WHERE random() < 0.3 -- 30% de chance de créer un avis
LIMIT 100;
...
WHERE id IN (SELECT id FROM imported_products);