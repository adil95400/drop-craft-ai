...
('ext_001', 'iPhone 15 Pro Max 256GB', 'Le dernier iPhone avec puce A17 Pro, écran Super Retina XDR de 6,7 pouces et système photo avancé', 1199.00, 'EUR', 'Smartphones', 'Apple', 'Apple', 'IPHONE15PM256', 
...
('ext_002', 'MacBook Air M3 13 pouces', 'MacBook Air avec puce M3, 8 Go de RAM et SSD de 256 Go. Ultra-léger et performant', 1299.00, 'EUR', 'Ordinateurs', 'Laptops', 'Apple', 'MBA13M3256', 
...
('ext_003', 'Samsung Galaxy S24 Ultra 512GB', 'Smartphone haut de gamme avec S Pen, écran Dynamic AMOLED 2X de 6,8 pouces', 1399.00, 'EUR', 'Smartphones', 'Android', 'Samsung', 'SGS24U512', 
...
('ext_004', 'AirPods Pro 3ème génération', 'Écouteurs sans fil avec réduction de bruit active et audio spatial', 279.00, 'EUR', 'Audio', 'Écouteurs', 'Apple', 'AIRPODSPRO3', 
...
('ext_005', 'PlayStation 5 Digital Edition', 'Console de jeux nouvelle génération avec SSD ultra-rapide', 449.99, 'EUR', 'Gaming', 'Consoles', 'Sony', 'PS5DIGITAL', 
...
('ext_007', 'Tesla Model Y Performance Accessoires', 'Kit d\'accessoires premium pour Tesla Model Y Performance', 899.00, 'EUR', 'Automobile', 'Accessoires', 'Tesla', 'TMYPERFORMKIT', 
...
('ext_008', 'Dell XPS 13 Plus i7 16GB', 'Ultrabook premium avec processeur Intel i7 12ème gen et écran InfinityEdge', 1599.00, 'EUR', 'Ordinateurs', 'Laptops', 'Dell', 'XPS13PLUSI7', 
...
('ext_010', 'Dyson V15 Detect Absolute', 'Aspirateur sans fil avec détection laser de la poussière', 749.99, 'EUR', 'Électroménager', 'Aspirateurs', 'Dyson', 'V15DETECTABS', 
...
INSERT INTO customers (name, email, phone, status, total_spent, total_orders, address, country, user_id)
SELECT 
  names.name, 
  names.email,
  names.phone,
...
FROM (
  VALUES 
    ('Jean Dupont', 'jean.dupont@email.com', '+33 1 23 45 67 89', '123 Rue de Rivoli', 'Paris', '75001'),
    ('Marie Martin', 'marie.martin@gmail.com', '+33 1 98 76 54 32', '456 Avenue des Champs-Élysées', 'Paris', '75008'),
    ('Pierre Durant', 'pierre.durant@hotmail.com', '+33 4 11 22 33 44', '789 Cours Mirabeau', 'Aix-en-Provence', '13100'),
    ('Sophie Leclerc', 'sophie.leclerc@yahoo.fr', '+33 2 55 66 77 88', '321 Quai des Grands Augustins', 'Nantes', '44000'),
    ('Thomas Moreau', 'thomas.moreau@outlook.com', '+33 5 99 88 77 66', '654 Boulevard Victor Hugo', 'Bordeaux', '33000'),
    ('Camille Bernard', 'camille.bernard@free.fr', '+33 3 44 55 66 77', '987 Rue de la République', 'Lyon', '69001'),
    ('Lucas Petit', 'lucas.petit@laposte.net', '+33 6 33 44 55 66', '147 Avenue Jean Médecin', 'Nice', '06000'),
    ('Emma Rousseau', 'emma.rousseau@sfr.fr', '+33 1 77 88 99 00', '258 Rue du Faubourg Saint-Honoré', 'Paris', '75008'),
    ('Hugo Lambert', 'hugo.lambert@orange.fr', '+33 4 22 33 44 55', '369 La Canebière', 'Marseille', '13001'),
    ('Léa Girard', 'lea.girard@bbox.fr', '+33 2 66 77 88 99', '741 Place Stanislas', 'Nancy', '54000')
) AS names(name, email, phone, street, city, postal_code)
WHERE EXISTS (SELECT 1 FROM auth.users);

-- Ajouter des commandes de démonstration
INSERT INTO orders (
  user_id, customer_id, order_number, status, total_amount, currency, 
  order_date, delivery_date, shipping_address, billing_address, items
)
SELECT 
  (SELECT id FROM auth.users LIMIT 1),
  c.id,
  'ORD-2024-' || LPAD((ROW_NUMBER() OVER())::text, 4, '0'),
  CASE 
  ...
  CASE 
    WHEN RANDOM() < 0.7 THEN NOW() - (RANDOM() * INTERVAL '60 days')
    ELSE NULL 
  END,
  c.address,
  c.address,
