 ...
 serve(async (req) => {
   ...
   try {
    const { shopDomain, accessToken, limit = 50, countOnly = false } = await req.json();
    ...
    // First, get the total product count
    const countUrl = `https://${shopDomain}/admin/api/${apiVersion}/products/count.json`;
