/**
 * @deprecated — Use `unified-ai/automation` instead.
...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch(`${Deno.env.get('SUPABASE_URL')!}/functions/v1/unified-ai/automation`, {
      method: 'POST',
    ...
    return new Response(data, { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json', 'X-Deprecated': 'Use unified-ai/automation' } })
  } catch (e) {
