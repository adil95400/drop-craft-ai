...
serve(async (req) => {
  ...
  try {
    ...
    if (!plan || !['pro', 'ultra_pro'].includes(plan)) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: { 
              name: priceData[plan as keyof typeof priceData].product_name,
      ...
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/modern/billing?success=true`,
      cancel_url: `${req.headers.get("origin")}/modern/billing?canceled=true`,
    });
