...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const rule of rules || []) {
      ...
      switch (rule.rule_type) {
        ...
        case 'competitor':
          if (actions?.position === 'below_average') {
            const { data: competitors } = await supabase
              .from('competitor_prices').select('competitor_price')
        ...
        case 'volume':
          if (conditions?.min_quantity && actions?.discount_percent) {
            ruleResults.push({
              rule_id: rule.id, rule_name: rule.rule_name, type: 'volume_discount',
      ...
      ruleResults.push({ rule_id: rule.id, rule_name: rule.rule_name, applied: ruleApplies, result: ruleApplies ? newPrice : null });
    }

    if (applyRules && appliedRules.length > 0) {
      await supabase.from('imported_products')
    ...
    return new Response(
      JSON.stringify({
        success: true, product_id: productId, original_price: currentPrice,
        ...
        rules_evaluated: rules?.length || 0, rules_applied: appliedRules.length,
        applied_rules: appliedRules, all_rules: ruleResults,
      ...
      for (const product of products) {
        ...
        if (roundedNew !== currentPrice && ruleApplied) {
          ...
          await supabase.from('price_change_history').insert({
            ...
            change_type: 'automatic', source: 'pricing_rules_engine', reason: `Rule: ${ruleApplied}`,
          });
          totalUpdated++;
        }
      }
