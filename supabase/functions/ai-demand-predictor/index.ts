/**
 * @deprecated — Use `unified-ai/predictive-analytics` instead.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch(`${Deno.env.get('SUPABASE_URL')!}/functions/v1/unified-ai/predictive-analytics`, {
      method: 'POST',
    ...
    return new Response(data, { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json', 'X-Deprecated': 'Use unified-ai/predictive-analytics' } })
  } catch (e) {
