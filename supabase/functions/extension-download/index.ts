...
function createZipFile(files: Record<string, string>): Uint8Array {
  console.log('Creating simple ZIP file without external dependencies...');
  ...
  // Write little-endian integers correctly
  ...
  console.log('Simple ZIP file created successfully');
  return zipFile;
}
...
// CRC32 calculation for valid ZIP files
...
serve(async (req) => {
  ...
  try {
    if (req.method === 'GET' || req.method === 'POST') {
      ...
      console.log('Generating ZIP file with simple method...');
      const startTime = Date.now();
      
      // Generate ZIP file with extension using simple method
