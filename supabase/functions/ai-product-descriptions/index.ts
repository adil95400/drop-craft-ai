/**
 * @deprecated — Use `unified-ai/generate-description` instead.
...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch(`${Deno.env.get('SUPABASE_URL')!}/functions/v1/unified-ai/generate-description`, {
      method: 'POST',
    ...
    return new Response(data, { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json', 'X-Deprecated': 'Use unified-ai/generate-description' } })
  } catch (e) {
