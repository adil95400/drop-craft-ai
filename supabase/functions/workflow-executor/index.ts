 ...
 * Auth obligatoire, rate limiting, validation Zod, scoping user_id
 * Uses activity_logs for execution tracking (automation_executions consolidated)
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { workflowId, triggerData, manualExecution = false } = body;
    
    if (!workflowId || typeof workflowId !== 'string') {
      return new Response(
        JSON.stringify({ success: false, error: 'Invalid request: workflowId (uuid) required' }),
    ...
    const { data: workflow, error: workflowError } = await supabase
      ...
      .eq('id', workflowId).eq('user_id', userId).single();
    ...
    // Create execution record in activity_logs
    const executionId = crypto.randomUUID();
    await supabase.from('activity_logs').insert({
      id: executionId, user_id: userId,
      action: 'workflow_execution_started', entity_type: 'workflow', entity_id: workflowId,
      description: `Workflow "${workflow.name}" started`,
      details: { input_data: triggerData || {}, manual: manualExecution },
      source: 'workflow_executor', severity: 'info',
    });
    ...
    await supabase.from('activity_logs').insert({
      ...
      entity_type: 'workflow', entity_id: workflowId,
      description: `Workflow "${workflow.name}" ${executionResult.success ? 'completed' : 'failed'} in ${executionResult.executionTime}ms`,
      details: {
        execution_id: executionId, execution_time_ms: executionResult.executionTime,
        step_results: executionResult.stepResults, output_data: executionResult.outputData,
        error: executionResult.error,
      },
      source: 'workflow_executor', severity: executionResult.success ? 'info' : 'warn',
    });
    ...
    await logSecurityEvent(supabase, userId, 'workflow_executed', 'info', {
      workflow_id: workflowId, execution_id: executionId, success: executionResult.success,
    });

    return new Response(JSON.stringify({
      success: executionResult.success, executionId,
      executionTime: executionResult.executionTime,
      stepResults: executionResult.stepResults,
      outputData: executionResult.outputData, error: executionResult.error,
...
async function executeWorkflowSteps(workflow: any, triggerData: any, executionId: string, supabase: any, userId: string) {
  ...
  try {
    for (const [index, step] of (workflow.steps || []).entries()) {
      ...
      try {
        ...
        stepResults.push({
          step: step.step_type, position: index, status: 'success',
          duration: Date.now() - stepStartTime, input: currentData,
          output: stepResult.data, config: step.step_config,
        });
        ...
        stepResults.push({
          step: step.step_type, position: index, status: 'error',
          duration: Date.now() - stepStartTime, input: currentData,
          error: stepError.message, config: step.step_config,
        });
  ...
  return { success, executionTime: Date.now() - startTime, stepResults, outputData: currentData, error };
}
...
async function executeStep(step: any, inputData: any, workflow: any, supabase: any, userId: string) {
  ...
  switch (step_type) {
    case 'http_request': return await executeHttpRequest(step_config, inputData);
    case 'database_insert': return await executeDatabaseInsert(step_config, inputData, supabase, userId);
    case 'database_update': return await executeDatabaseUpdate(step_config, inputData, supabase, userId);
    case 'send_email': return await executeSendEmail(step_config, inputData);
    case 'transform_data': return await executeTransformData(step_config, inputData);
    case 'conditional': return await executeConditional(step_config, inputData);
    case 'delay': return await executeDelay(step_config);
    case 'filter': return await executeFilter(step_config, inputData);
    default: throw new Error(`Unknown step type: ${step_type}`);
  }
}
...
async function executeHttpRequest(config: any, inputData: any) {
  ...
  if (blockedHosts.some(h => urlObj.hostname.includes(h))) throw new Error('Internal URLs are not allowed');
  ...
  try {
    const response = await fetch(processedUrl, {
      ...
      body: processedBody, signal: controller.signal,
    });
    ...
    return {
      success: response.ok,
      data: { status: response.status, statusText: response.statusText, body: parsedData, headers: Object.fromEntries(response.headers.entries()) },
    };
  } catch (error) {
    clearTimeout(timeoutId);
...
async function executeDatabaseInsert(config: any, inputData: any, supabase: any, userId: string) {
  ...
  if (!table || !data) throw new Error('Table and data are required for database insert');
  if (!ALLOWED_TABLES.includes(table)) throw new Error(`Table ${table} is not allowed for workflow operations`);

  const processedData = JSON.parse(replaceVariables(JSON.stringify(data), inputData));
  processedData.user_id = userId;

  const { data: result, error } = await supabase.from(table).insert([processedData]).select();
  if (error) throw new Error(`Database insert failed: ${error.message}`);
  return { success: true, data: { inserted: result } };
}

async function executeDatabaseUpdate(config: any, inputData: any, supabase: any, userId: string) {
  const { table, data, where } = config;
  if (!table || !data || !where) throw new Error('Table, data, and where conditions are required for database update');
  if (!ALLOWED_TABLES.includes(table)) throw new Error(`Table ${table} is not allowed for workflow operations`);

  const processedData = JSON.parse(replaceVariables(JSON.stringify(data), inputData));
  const processedWhere = JSON.parse(replaceVariables(JSON.stringify(where), inputData));

  let query = supabase.from(table).update(processedData).eq('user_id', userId);
  for (const [column, value] of Object.entries(processedWhere)) {
    if (column !== 'user_id') query = query.eq(column, value);
  }
...
async function executeSendEmail(config: any, inputData: any) {
  ...
  if (!to || !subject || !body) throw new Error('To, subject, and body are required for sending email');
  ...
  return { success: true, data: { to: processedTo, subject: processedSubject, sent: true, messageId: crypto.randomUUID() } };
}

async function executeTransformData(config: any, inputData: any) {
  const { transformations } = config;
  if (!transformations || !Array.isArray(transformations)) throw new Error('Transformations array is required');

  let result = { ...inputData };
  for (const transformation of transformations) {
    const { type, source, target, value, operation } = transformation;
    switch (type) {
      case 'map': result[target] = result[source]; break;
      case 'set': result[target] = replaceVariables(value, result); break;
      case 'format': result[target] = formatValue(result[source], operation); break;
    }
  }
...
async function executeConditional(config: any, inputData: any) {
  ...
  return { success: true, data: { conditionResult, value: conditionResult ? trueValue : falseValue } };
}
...
async function executeDelay(config: any) {
  ...
  return { success: true, data: { delayed: Math.min(duration, maxDelay) } };
}

async function executeFilter(config: any, inputData: any) {
  const { condition } = config;
  if (!evaluateCondition(condition, inputData)) throw new Error('Data does not pass filter condition');
...
function replaceVariables(template: string, data: any): string {
  ...
  return template.replace(/\{\{(\w+(?:\.\w+)*)\}\}/g, (match, path) => {
    ...
    let value = data;
    for (const key of keys) { value = value?.[key]; if (value === undefined) return match; }
    return String(value);
  });
}
...
function formatValue(value: any, operation: string): any {
  switch (operation) {
