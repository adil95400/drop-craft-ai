 ...
 * SECURITY: Requires authentication, validates all inputs with Zod
 */

import { verifyAuth } from '../_shared/secure-auth.ts'
import { handleError } from '../_shared/error-handler.ts'
import { aiContentInputSchema, validateJsonBody } from '../_shared/validation.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`AI Content request from user: ${user.id}`)
    ...
    const validated = await validateJsonBody(req, aiContentInputSchema)
    const { type, prompt, keywords, language, action, content, tone, contentType } = validated
