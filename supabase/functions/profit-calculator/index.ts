...
import { validateInput, profitCalculatorSchema } from '../_shared/input-validation.ts'
...
Deno.serve(withErrorHandler(async (req) => {
  ...
  const { userId, productId, sellingPrice, costPrice, additionalCosts = {} } = 
    validateInput(profitCalculatorSchema, rawBody)
