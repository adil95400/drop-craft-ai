...
serve(async (req) => {
  ...
  try {
    ...
    const body = await req.json() as OrderSyncRequest
    ...
    const { integration_id, platform, direction = 'bidirectional', order_ids } = body

    console.log(`📋 Order sync starting for user ${userId}, direction: ${direction}`)
